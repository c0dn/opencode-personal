...
export function define<const Type extends string, Fields extends Schema.Struct.Fields>(input: {
  ...
  if (input.sync === undefined) {
    if (existing) throw new Error(`Event type ${input.type} is already registered`)
    registry.set(input.type, definition)
    return definition as Schema.Schema<Payload<Definition<Type, Schema.Struct<Fields>>>> &
      Definition<Type, Schema.Struct<Fields>>
  }

  if (existing && existing.sync === undefined) throw new Error(`Event type ${input.type} is already registered`)
  const syncType = versionedType(input.type, input.sync.version)
  if (syncRegistry.has(syncType)) throw new Error(`Event type ${syncType} is already registered`)
  const existingSync = existing?.sync
  if (existingSync === undefined || input.sync.version > existingSync.version) registry.set(input.type, definition)
  syncRegistry.set(
    syncType,
...
export function encodePayload<D extends Definition>(definition: D, payload: Payload<D>): EncodedPayload<D> {
  return { ...payload, data: asRecord(Schema.encodeUnknownSync(syncCodec(definition))(payload.data)) }
}
    ...
    Effect.gen(function* () {
      ...
      function commitSyncEvent(
        ...
        return Effect.gen(function* () {
          const definition = input?.definition ?? registry.get(event.type)
        ...
        return Effect.gen(function* () {
          ...
          if (!definition) {
            ...
            const payload = {
              ...
              data: definition.decode(normalizeLegacyReplayData(event.data)),
            } as Payload
...
function normalizeLegacyReplayData(value: unknown): unknown {
  if (!value || typeof value !== "object") return value
  if (Array.isArray(value)) return value.map(normalizeLegacyReplayData)
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => {
      if (key === "timestamp" && typeof item === "string") {
        const millis = new Date(item).getTime()
        if (Number.isFinite(millis)) return [key, millis]
      }
      return [key, normalizeLegacyReplayData(item)]
    }),
  )
}
