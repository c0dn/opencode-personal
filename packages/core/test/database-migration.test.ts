  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'session_mailbox'`),
        ).toEqual({ name: "session_mailbox" })
        expect(yield* db.get(sql`SELECT count(*) as count FROM migration`)).toEqual({ count: 36 })
        expect(
          yield* db.all(
