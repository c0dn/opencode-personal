...
| `bifrost` | Object  |          | Bifrost gateway virtual-key auth and client/tool filtering.                         |
...
For a Bifrost MCP gateway, use `bifrost` to send an optional virtual key and optionally narrow the visible clients or tools.
...
```jsonc title="opencode.jsonc"
{
  ...
  "mcp": {
    "bifrost-files": {
      ...
      "bifrost": {
        "virtualKey": "{env:BIFROST_VIRTUAL_KEY}",
        "includeClients": ["filesystem"],
        "includeTools": ["filesystem-read_file", "filesystem-list_directory"]
      }
    }
  }
}
...
You can also use include filters without a virtual key when Bifrost allows unauthenticated `/mcp` requests:
...
`virtualKey` is sent as `x-bf-vk` when configured. `includeClients` and `includeTools` are sent as Bifrost include headers and can be used by themselves if the gateway allows unauthenticated MCP. If Bifrost enforces authentication, configure `virtualKey` or raw `headers`. These include filters only narrow what Bifrost exposes for this request; authentication or the virtual key's MCP allowlist is the authorization boundary. OpenCode disables OAuth auto-detection for native Bifrost config unless you explicitly set `oauth`.
